...
def  run(
    ...
    #1 对传入参数的额外判断
    source = str(source) #source路径强制转为字符串类型
    save_img = not nosave and not source.endswith('.txt')  # save inference images， nosave 和 source是否以txt结尾
    is_file = Path(source).suffix[1:] in (IMG_FORMATS + VID_FORMATS)  # source的结尾从'.'开始，看格式是否在 IMG_FORMATS + VID_FORMATS之中
    is_url = source.lower().startswith(('rtsp://', 'rtmp://', 'http://', 'https://'))# 转小写，是不是以http等开头的
    webcam = source.isnumeric() or source.endswith('.txt') or (is_url and not is_file)# 判断传入的source参数 是不是摄像头（0）， 是不是txt结尾的，是不是网络流， 是不是文件
    if is_url and is_file: # 是网络流且是文件则下载
        source = check_file(source)  # download
    #2 新建保存结果的文件夹
    # Directories
    save_dir = increment_path(Path(project) / name, exist_ok=exist_ok)  # exp文件夹递增
    (save_dir / 'labels' if save_txt else save_dir).mkdir(parents=True, exist_ok=True)  # make dir 如果save_txt则exp中加一个label
    #3 加载模型权重
    # Load model
    device = select_device(device) # 选择cpu或gpu
    ...
    seen, windows, dt = 0, [], [0.0, 0.0, 0.0] #dt seen存中间结果信息
    for path, im, im0s, vid_cap, s in dataset:
        ...
        # Inference
        visualize = increment_path(save_dir / Path(path).stem, mkdir=True) if visualize else False # visualize 是否保存特征图
        ...
        # NMS 非极大值过滤
        pred = non_max_suppression(pred, conf_thres, iou_thres, classes, agnostic_nms, max_det=max_det) # 根据置信度做非极大值过滤
        print(pred)
        dt[2] += time_sync() - t3
        ...
        for i, det in enumerate(pred):  # per image, torch.Size([5, 6])
            ...
            p = Path(p)  # to Path
            save_path = str(save_dir / p.name)  # im.jpg  图片保存路径
            txt_path = str(save_dir / 'labels' / p.stem) + ('' if dataset.mode == 'image' else f'_{frame}')  # im.txt
            s += '%gx%g ' % im.shape[2:]  # print string
            gn = torch.tensor(im0.shape)[[1, 0, 1, 0]]  # normalization gain whwh 获得原图宽高大小
            imc = im0.copy() if save_crop else im0  # for save_crop  裁剪box框
            annotator = Annotator(im0, line_width=line_thickness, example=str(names))
            if len(det):
                # Rescale boxes from img_size to im0 size
                det[:, :4] = scale_coords(im.shape[2:], det[:, :4], im0.shape).round()# 画的box是640*640的，坐标不能直接画在img0
                ...
                for *xyxy, conf, cls in reversed(det):
                    if save_txt:  # Write to file box保存为txt
...
def main(opt):
    check_requirements(exclude=('tensorboard', 'thop')) #检测依赖包
    run(**vars(opt))
