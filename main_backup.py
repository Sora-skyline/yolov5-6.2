...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # 命令行参数转为变量
        self.weights, self.view_img, self.save_txt, self.imgsz, self.device = self.opt.weights, self.opt.view_img, self.opt.save_txt, self.opt.imgsz, self.opt.device
        self.project, self.name, self.exist_ok, self.dnn, self.data, self.half, self.nosave = self.opt.project, self.opt.name, self.opt.exist_ok, self.opt.dnn, self.opt.data, self.opt.half, self.opt.nosave
        self.augment, self.conf_thres, self.iou_thres, self.classes, self.agnostic_nms, self.max_det = self.opt.augment, self.opt.conf_thres, self.opt.iou_thres, self.opt.classes, self.opt.agnostic_nms, self.opt.max_det
        self.line_thickness, self.hide_labels, self.hide_conf, self.visualize = self.opt.line_thickness, self.opt.hide_labels, self.opt.hide_conf, self.opt.visualize
        # Load model
        self.device = select_device(self.device)
        self.model = DetectMultiBackend(self.weights, device=self.device, dnn=self.dnn, data=self.data, fp16=self.half)  # 选择模型的后端框架
    ...
    def imgload(self):
        ...
        with torch.no_grad():
            im = letterbox(im0, self.imgsz, stride)[0]  # 改变图片尺寸， 模型只能输入640
            # 640 *640            32                 true
            # Convert
            im = im.transpose((2, 0, 1))[::-1]  # HWC to CHW, BGR to RGB , [::-1] 代表顺序相反操作
            im = np.ascontiguousarray(im)
            im = torch.from_numpy(im).to(self.device)  # 把img转成pytorch支持的格式
            im = im.half() if self.model.fp16 else im.float()  # uint8 to fp16/32 ,有没有用到半精度
            im /= 255  # 0 - 255 to 0.0 - 1.0 归一化
            if len(im.shape) == 3:
                im = im[None]  # expand for batch dim
            # Inference
            visualize = increment_path(save_dir / Path(path).stem,
                                       mkdir=True) if self.visualize else False  # visualize 是否保存特征图
            pred = self.model(im, augment=self.augment, visualize=visualize)  # 模型所有检测框
            # [tensor([[3.98096e+02, 2.34295e+02, 4.80189e+02, 5.20510e+02, 8.96172e-01, 0.00000e+00],

            # NMS 非极大值过滤
            pred = non_max_suppression(pred, self.conf_thres, self.iou_thres, self.classes, self.agnostic_nms,
                                       max_det=self.max_det)  # 根据置信度做非极大值过滤
            # Process predictions
    ...
    def videoload(self):
        if not self.timer_video.isActive():
            ...
            if flag == False:
                ...
                self.timer_video.start(10)  # 定时器读取视频流
    ...
    def openframe(self):
        ...
        # 执行模型推理过程，并画检测框
        if ret:
            im = letterbox(im0, new_shape=self.imgsz)[0]
            ...
            if len(im.shape) == 3:
                im = im[None]  # expand for batch dim
            # Inference
            pred = self.model(im, augment=self.augment, visualize=self.visualize)  # 模型所有检测框
            # [tensor([[3.98096e+02, 2.34295e+02, 4.80189e+02, 5.20510e+02, 8.96172e-01, 0.00000e+00],
            # NMS 非极大值过滤
            pred = non_max_suppression(pred, self.conf_thres, self.iou_thres, self.classes, self.agnostic_nms,
                                       max_det=self.max_det)  # 根据置信度做非极大值过滤
